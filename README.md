...
- **Corner controls** — a tile's controls appear in the corner on hover or keyboard focus. When the target has an external link an ↗ open-external button sits beside the ⋮ _actions_ menu, which opens on hover/focus with ⧉ show-in-popup, ⊟ toggle window/bare, × remove.
...
- **Attach a file — drop or paste** — drop an image (or other media) onto a tile, or click the tile's 📋 corner button and press Ctrl+V to paste one from the clipboard. The file is uploaded to the filesystem location *derived from that tile's target* — a configured [rimir/file-upload](https://github.com/rimir-cc/tw-file-upload) scattered location when the target's namespace matches one (e.g. a `work/…` target files into the `work` location alongside that item), otherwise the default `files` location — and added as a new tile beside it. Dropping/pasting on a group (or its canvas) files into the group's *context* location and adds a child; the plain canvas uses `files`. Requires the file-upload plugin.
- **Drop onto a collapsed tile** — a collapsed tile or group is still a drop/paste target: drop a tiddler or file onto a collapsed *group* to add it as a child (the group then expands), or drop/paste a file onto a collapsed *content tile* to file it in the tile's derived location. No extra chrome — the box is just outlined while you drag over it.
...
- **Link a group to a tiddler (its context)** — connect a group to a tiddler as its _implicit context_: open the group's ⋮ menu → _Context…_ and name a tiddler (or drop a tiddler onto the group header). The group adopts that tiddler's caption (editable independently afterwards), shows its [`rimir/kind`](https://github.com/rimir-cc/tw-kind) icon in the header (click to open the tiddler in the story river), and — when the context is a kind instance — gains the same ✎ _entity_ popup a content tile has, so **Add child** creates instances with `parent` pre-set to the context _and drops their tiles inside this group_. _Unlink_ from the same menu (the caption is kept).
- **Click to open** — a click (not a drag) opens the tile's target tiddler in the story river.
- **Open a link in a new browser tab** — if a tile's target carries a `url` (or `_canonical_uri`) field, its **caption is rendered as a link** (hover to see the address in the browser status bar; click to open it in a new tab). The tile also shows an ↗ action, and **Ctrl/Cmd-clicking** the tile does the same (an uploaded image or file tile opens its raw file this way). Plain click elsewhere on the tile still opens the target in the wiki. The fields consulted and their priority are configurable.
- **Open a tiddler in a new browser tab** — for a tile whose target has _no_ external link, the ⋮ menu offers an ↗ item that opens the target tiddler itself in a new tab, via its permalink.
...
- **Ctrl-drag a group to reference it** — the same gesture on a *group* drops a live **reference**: a linked copy (brighter grey, with a 🔗 badge) that shows the same contents, name and icon. Work in any copy as if it were the original — renaming it, changing its icon or context, and adding, removing or re-arranging its children all reflect on the original and every reference at once. Each copy keeps its own position, size and collapsed state. Remove a reference and the original stays; remove the original and it asks first, telling you how many references go with it. A reference's ↗ control (revealed on hover, beside its ⋮ menu) **jumps to the original** — opening its parent groups, scrolling to it and flashing it red.
- **Find a tile** — the *Find* box above the canvas locates a tile by caption: type text and each *Find* jumps to the next match — opening its parent groups, scrolling to it and flashing it red.
